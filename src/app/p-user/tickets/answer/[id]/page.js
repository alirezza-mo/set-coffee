...
import Answer from "@/component/templates/p-user/tickets/Answer";
...
const page = async ({ params }) => {
  ...
  const ticket = await TicketModel.findOne({ _id: ticketID })
    .populate("user", "name")
    .lean();
  const answerTicket = await TicketModel.findOne({ mainTicket: ticket._id }).lean()
    ...
    <Layout>
      <main className={styles.container}>
        ...
        <div>
          ...
          {answerTicket && <Answer {...answerTicket} type="admin" />}
